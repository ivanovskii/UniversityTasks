...
var
  competition: array [0..1000] of participant; //Массив участников
  f1, f2: text;
...
begin
  ...
  begin
    ...
    competition[i].name := copy(str, 1, pos(' ', str)); //Запоминаем имя
    delete(str, 1, length(competition[i].name)); //Удаляем из строки
    competition[i].fname := copy(str, 1, pos(' ', str));
    delete(str, 1, length(competition[i].fname));
    val(str, competition[i].point, err); //Преобразуем оставшуюся часть строки в число и запоминаем как очки
    i += 1;
  end;
  
  for var h := 0 to i - 1 do //Сортировка по возрастанию пузырьком по очкам
